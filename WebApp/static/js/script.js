...
Object.entries(uniforms).forEach((u, v) => {
  ...
  if (prop.type === 'COLOUR') {
    gui.addColor(prop, 'v').name(u[0]).onChange(v => {
      updateUniform(u[0], v);
    });
  } else {
    var controller = gui.add(prop, 'v', prop.min, prop.max).name(u[0]).onChange(v => {
      updateUniform(u[0], v);
    });
...
gui.hide();
export default {controllers,gui};
